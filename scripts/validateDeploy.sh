...
metalistSfdx=$(git diff --name-only HEAD $destination  -- force-app/main/default | tr '\n' ',' | sed 's/.$//' )
...
then
    ...
    sfdx force:source:deploy -x package/package.xml --targetusername sfdx-ci -l RunSpecifiedTests -r "$(<TEST_CLASSES_MERGED)" --checkonly --verbose
elif [ -z "$(<TEST_CLASSES_MERGED)" ] && [ ! -z "$metalistSfdx" ] && [ "$destination" != "origin/main" ]
then
    echo "IT WILL DEPLOY WITHOUT ANY TEST (SANDBOX)"
    sfdx force:source:deploy -x package/package.xml --targetusername sfdx-ci --checkonly --verbose
elif [ -z "$(<TEST_CLASSES_MERGED)" ] && [ ! -z "$metalistSfdx" ] && [ "$destination" == "origin/main" ]
then
    echo "IT WILL DEPLOY RUNNING LOCAL TESTS (PRODUCTION)"
    # In prod we need to run some test class
    sfdx force:source:deploy -x package/package.xml --targetusername sfdx-ci -l RunLocalTests --checkonly --verbose
else
